...
PairGroupList * getValidInitialEllipseSet(double * lines, int line_num, vector<vector<int>> * groups,
	...
	for (i = 0; i < groupsNum; i++)
	{
		if (coverages[i] >= M_4_9_PI)//!!!!!!eros 2020年3月17日 当组的覆盖角度>= 4pi/9 = 80°, 我们认为具有很大的显著性，可直接拟合提取
		{
			...
			if (specified_polarity == 0 || (lines[(*groups)[i][0] * 8 + 7] == specified_polarity))
			{
				...
				info = calcEllipseParametersAndValidate(lines, line_num, groups, i, -1, (fitMatrixes + i * 36),
					NULL, angles, distance_tolerance, supportInliersNum, &ellipara);
				if (info == FALSE)
				{
					continue;
		...
		for (j = i + 1; j < groupsNum; j++)
		{
			...
			if (specified_polarity == 0 || (lines[(*groups)[i][0] * 8 + 7] == specified_polarity))
			{
				...
				if (lines[(*groups)[i][0] * 8 + 7] == lines[(*groups)[j][0] * 8 + 7] && ((*groups)[i].size() + (*groups)[j].size()) >= 3)
				{
					...
					if (regionLimitation(pointG1s, g1s_ls_dir, pointG1e, g1e_ls_dir, pointG2s, g2s_ls_dir,
						pointG2e, g2e_ls_dir, polarity, -3 * distance_tolerance))//都在彼此的线性区域内
					{
						//if ( i == 2)
						//	drawPairGroup(img,lines,(*groups),i,j);

						if (calcEllipseParametersAndValidate(lines, line_num, groups, i, j, (fitMatrixes + i * 36), 
							(fitMatrixes + j * 36), angles, distance_tolerance, supportInliersNum, &ellipara))//二次一般方程线性求解，线段的内点支持比例
						{
							PairGroupNode * node = (PairGroupNode*)malloc(sizeof(PairGroupNode));
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	// out reg 标记 regiongrow 一维数组  //申请与降采样后图像一样大小的int类型的内存，该内存的作用是将检测到的线段序号标到相应的图像格子里
	...
	// pairGroupList 组
	pairGroupList = getValidInitialEllipseSet(out, n, &groups, coverages, angles, distance_tolerance, specified_polarity);// n 150 groups 13
	if (pairGroupList != NULL)
	{
		//The number of initial ellipses：23
